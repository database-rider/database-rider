...
A dataset provider is a *Java class responsible for defing a dataset* instead of having `yml`, `json`, `xml` files representing your datasets. Following are the steps for creating and using a dataset provider:
...
public class UserDataSetProvider implements DataSetProvider {
        ...
        public IDataSet provide() throws DataSetException {
            RiderDataSetBuilder builder = new RiderDataSetBuilder(true); <1>
...
----
<1> The `boolean` parameter means table name and columns will be in uppercase on generated dataset.
+
The above DataSet provider will generate a dataset for `USER` table with 2 rows.
+
