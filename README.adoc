...
To ignore how order in expected dataset one can use **orderBy** attribute in expected dataset.
...
Note on the example above that the order of insertion is different from declared on the dataset but the test passes because of `orderBy`.
...
NOTE: It `don't need` to have ALL rows as in the default behaviour of expected datataset.
...
NOTE: This test will pass if, and only if, database state after test contains a row in user table with id=3 and name = @dbrider. Other rows doesn't matter.
