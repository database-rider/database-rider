...
public class UserDataSetProvider implements DataSetProvider {
        ...
        public IDataSet provide() throws DataSetException {
            ...
            builder.table("user").column("id", 1)
                    .column("name", "@dbunit")
                    .row() //starts a new row for user table
                    .column("id", 2)
                    .column("name", "@dbrider")
                    .table("tweet")// starts adding rows to tweet table
                    .column("id", "abcfds")
                    .column("content", "database rider rules!")
            return builder.build();
        }
    }
----
+
The above DataSet provider will generate a dataset for `USER` table with 2 rows and with tweet table with one row.
+
