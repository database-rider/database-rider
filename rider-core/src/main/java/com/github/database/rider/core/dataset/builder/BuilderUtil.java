    ...
    public static String getColumnNameFromMetaModel(Attribute column) {
        ...
        try {
            if (isEclipseLinkOnClasspath()) {
                ...
                AbstractEntityPersister entityMetadata = (AbstractEntityPersister) ((org.hibernate.metamodel.MappingMetamodel)em().getEntityManagerFactory().unwrap(SessionFactory.class).getMetamodel()).getEntityDescriptor(column.getJavaMember().getDeclaringClass());
